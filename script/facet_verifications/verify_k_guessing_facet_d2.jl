...
# verification script: verifies all k-guessing facets with Y ∈ [4, Y_max] and d = 2
function verify_k_guessing_facet_d2(Y_max :: Int64)
    @testset "Testing all k-guessing facets with `k = 2` and `Y ≤ $Y_max`." begin
        @testset "Verifying Y = $Y, d = 2, and k = $(Y - 2)" for Y in 4:Y_max
            d = 2
            k = Y - d
